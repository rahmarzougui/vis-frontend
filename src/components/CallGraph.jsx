...
const CallGraph = ({ selectedFunction = null, onNodeClick = null }) => {
  ...
  const applyHighlight = useCallback((selectedId, depth) => {
    ...
    nodeSelection
      .style('display', d => {
        if (!selectedId) return 'block';
        return nodesWithinDepth.has(d.id) ? 'block' : 'none';
      })
      .attr('opacity', d => {
        if (!selectedId) return 1;
        return nodesWithinDepth.has(d.id) ? 1 : 0;
      })
      .select('circle')
      .attr('fill', d => {
    ...
    linkSelection
      .style('display', d => {
        ...
        return highlightedEdgeKeys.has(key) ? 'block' : 'none';
      })
      .attr('opacity', d => {
        if (!selectedId) return 0.6;
        const key = createEdgeKey(getDatumId(d.source), getDatumId(d.target));
        return highlightedEdgeKeys.has(key) ? 0.9 : 0;
      })
      .attr('stroke', d => {
